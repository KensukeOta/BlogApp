    ...
    <div class="card-body p-3">
        ...
        <div class="d-flex justify-content-between">
            <p class="card-text text-muted m-0">
                ...
                by <img src="/img/noimage.png" alt="image" class="post-user_image mr-1"><a href="{{ action('UserController@home', $post->user->name) }}" class="text-muted">{{ $post->user->name }}</a>
                @else
                by <img src="{{ $post->user->path }}" alt="image" class="post-user_image mr-1"><a href="{{ action('UserController@home', $post->user->name) }}" class="text-muted">{{ $post->user->name }}</a>
                @endif
            </p>
            @if (Auth::check())
